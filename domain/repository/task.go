...
type TaskRepo interface {
	...
	UpdateTaskDueDate(ctx context.Context, task *entity.Task) error
	UpdateTaskDescription(ctx context.Context, task *entity.Task) error
	UpdateTaskName(ctx context.Context, task *entity.Task) error

	// 按状态查询今天到期的所有任务 ，-1就是不区分
	FindDueTasksByStatus(ctx context.Context, status int) ([]*entity.Task, error)
